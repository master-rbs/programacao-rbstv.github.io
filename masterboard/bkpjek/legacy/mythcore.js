...
*/
    for (var i in myth.badges) {
        document.getElementById(_database/myth.tagId).innerHTML += ' <img class="badgeicon" title="' + _database/myth.badges[i][0] + '" src="' + PATH + _database/myth.badges[i][1] + '.png"> ';
    }
    document.getElementById(_database/myth.tagId1).innerHTML += _database/myth.badges.length;
}
...
function toggleSort(type){
    ...
    if (type == 'ano') {
        ...
        if (checkAno) {
            tinysort('div#tabelas>table');
            tinysort('div#tabelas>table',{selector:'strong._database/mythano'});
            checkTier = false;
            document.getElementById('titl').innerHTML = "Mostrando lista por Ano (Decrescente)";
        }else{
            tinysort('div#tabelas>table',{order:'desc'});
            tinysort('div#tabelas>table',{selector:'strong._database/mythano',order:'desc'});
            checkTier = true;
